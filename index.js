const { intervals, token, twitchClientID } = require("./config.json");
/* jshint esversion: 6 */
  ...
  if (message.content[0] == server.prefix) {
    ...
    if (message.content.substring(1, 7) == "remove") {
      if (permission) {
        ...
        message.reply(":no_entry: You have permission to do that.");
      }
      ...
      if (message.guild.owner == message.member) {
        ...
        msg += ":x: You are not the server owner.";
      }
      message.reply(msg);
    } else {
      message.reply(
...
Client.login(token).then((token) => {
  if (token) {
    print( "Twitch Announcer Ready, " + Client.user.tag + " is logged in!");
